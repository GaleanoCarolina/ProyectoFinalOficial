...
def agregar_stock_producto(tabla, entrada_agregar_stock, entrada_existencia):
    try:
        seleccion = tabla.focus()
        if not seleccion:
            raise ValueError("Selecciona un producto en la tabla primero.")
        valores = tabla.item(seleccion, "values")
        if not valores:
            raise ValueError("No se logró leer la fila seleccionada.")
        codigo_sel, _nom, existencia_actual, _prov, _prec = valores


        cant_extra_txt = entrada_agregar_stock.get().strip()
        if cant_extra_txt == "":
            raise ValueError("Ingresa la cantidad a agregar.")
        cant_extra = int(float(cant_extra_txt))
        if cant_extra <= 0:
        ...
        idx = df.index[df["codigo"] == str(codigo_sel)]
        if idx.empty:
            raise ValueError("No se encontró el producto en el inventario.")

        nueva_existencia = int(float(existencia_actual)) + cant_extra
        df.at[idx[0], "existencia"] = nueva_existencia

        escribir_inventario(df)
        listar_productos(tabla)


        entrada_existencia.config(state="normal")
        entrada_existencia.delete(0, tk.END)
        entrada_existencia.insert(0, str(nueva_existencia))

        entrada_agregar_stock.delete(0, tk.END)
        entrada_agregar_stock.config(state="disabled")

        messagebox.showinfo("Éxito", f"Se agregaron {cant_extra} unidades al stock.")
    except Exception as e:
        messagebox.showerror("Error", str(e))
...
def pestaña_inventario(notebook):
    ...
    entrada_agregar_stock = tb.Entry(formulario, width=8)
    # Nuevo campo de texto: Categoría
    tb.Label(formulario, text="Categoría:").grid(row=2, column=0, padx=4, pady=4, sticky="e")
    entrada_categoria = tb.Entry(formulario, width=20)
    entrada_categoria.grid(row=1, column=1, padx=4, pady=4)

    tb.Label(formulario, text="Código:").grid(row=0, column=0, padx=4, pady=4, sticky="e")
    entrada_codigo.grid(row=0, column=1, padx=4, pady=4)

    tb.Label(formulario, text="Nombre:").grid(row=0, column=2, padx=4, pady=4, sticky="e")
    entrada_nombre.grid(row=0, column=3, padx=4, pady=4)

    tb.Label(formulario, text="Existencia actual:").grid(row=0, column=4, padx=4, pady=4, sticky="e")
    entrada_existencia.grid(row=0, column=5, padx=4, pady=4)

    tb.Label(formulario, text="Proveedor:").grid(row=1, column=0, padx=4, pady=4, sticky="e")
    entrada_proveedor.grid(row=1, column=1, padx=4, pady=4)

    tb.Label(formulario, text="Precio:").grid(row=1, column=2, padx=4, pady=4, sticky="e")
    entrada_precio.grid(row=1, column=3, padx=4, pady=4)

    tb.Label(formulario, text="Agregar al stock (+):").grid(row=1, column=4, padx=4, pady=4, sticky="e")
    entrada_agregar_stock.grid(row=1, column=5, padx=4, pady=4)





    def llenar_campos_producto(_event=None):
        seleccion = tabla.focus()
        if not seleccion:
            return
        valores = tabla.item(seleccion, "values")
        if not valores:
            return
        cod, nom, exis, prov, prec = valores

        entrada_codigo.config(state="normal")
        entrada_codigo.delete(0, tk.END)
        entrada_codigo.insert(0, cod)
        entrada_codigo.config(state="disabled")

        entrada_nombre.delete(0, tk.END)
        entrada_nombre.insert(0, nom)

        entrada_existencia.config(state="normal")
        entrada_existencia.delete(0, tk.END)
        entrada_existencia.insert(0, exis)

        entrada_proveedor.delete(0, tk.END)
        entrada_proveedor.insert(0, prov)

        entrada_precio.delete(0, tk.END)
        entrada_precio.insert(0, prec)

        # habilitar el campo para sumar stock
        entrada_agregar_stock.config(state="normal")
        entrada_agregar_stock.delete(0, tk.END)

    def limpiar_campos_producto():
# Limpiamos las entradas cajas de texto
        entrada_codigo.config(state="normal")
        entrada_codigo.delete(0, tk.END)
    ...
    tabla.bind("<<TreeviewSelect>>", llenar_campos_producto)
    ...
    ).grid(row=2, column=0, pady=6, padx=4)
    ...
    tb.Button(
        ...
        command=lambda: (
            actualizar_producto(
                tabla,
                entrada_nombre.get(),
                entrada_existencia.get(),
                entrada_proveedor.get(),
                entrada_precio.get()
            )
        )
    ).grid(row=2, column=1, pady=6, padx=4)

# sumar stock awui
    tb.Button(
        formulario, text="Agregar al stock", bootstyle="warning",
        command=lambda: agregar_stock_producto(
            tabla,
            entrada_agregar_stock,
            entrada_existencia
        )
    ).grid(row=2, column=2, pady=6, padx=4)

    tb.Button(
        formulario, text="Eliminar selección", bootstyle="danger",
        command=lambda: (eliminar_producto(tabla))
    ).grid(row=2, column=3, pady=6, padx=4)

    tb.Button(
        formulario, text="Refrescar", bootstyle="secondary",
        command=lambda: (listar_productos(tabla), limpiar_campos_producto())
    ).grid(row=2, column=4, pady=6, padx=4)



    pestaña.bind("<Visibility>", lambda _e: (listar_productos(tabla), limpiar_campos_producto()))
...
def pestaña_clientes(notebook):
    ...
    def llenar_campos_cliente(_event=None):
        """Copia los datos de la fila seleccionada a las cajas y bloquea código."""
        seleccion = tabla.focus()
        if not seleccion:
            return
        valores = tabla.item(seleccion, "values")
        if not valores:
            return
        cod, nom, dire = valores

        entrada_codigo.config(state="normal")
        entrada_codigo.delete(0, tk.END)
        entrada_codigo.insert(0, cod)
        entrada_codigo.config(state="disabled")  # bloquear edición de coodigo

        entrada_nombre.delete(0, tk.END)
        entrada_nombre.insert(0, nom)

        entrada_direccion.delete(0, tk.END)
        entrada_direccion.insert(0, dire)
    ...
    tabla.bind("<<TreeviewSelect>>", llenar_campos_cliente)

    tb.Button(formulario, text="Crear", bootstyle="success",
              command=lambda: (crear_cliente(entrada_codigo.get(), entrada_nombre.get(), entrada_direccion.get(), tabla))
    ).grid(row=1, column=1, pady=6)

    tb.Button(formulario, text="Actualizar selección", bootstyle="info",
              command=lambda: (actualizar_cliente(tabla, entrada_nombre.get(), entrada_direccion.get()))
    ).grid(row=1, column=3, pady=6)

    tb.Button(formulario, text="Eliminar selección", bootstyle="danger",
              command=lambda: (eliminar_cliente(tabla))
    ).grid(row=1, column=5, pady=6)

    tb.Button(formulario, text="Refrescar", bootstyle="secondary",
              command=lambda: (listar_clientes(tabla), limpiar_campos_cliente())
    ).grid(row=1, column=6, pady=6)

    listar_clientes(tabla)
    pestaña.bind("<Visibility>", lambda _e: (listar_clientes(tabla), limpiar_campos_cliente()))
...
def precio_producto_por_codigo(codigo: str) -> float:
    inv = leer_inventario().copy()
    inv["codigo"] = inv["codigo"].astype(str)
    fila = inv.loc[inv["codigo"] == str(codigo)]
    if fila.empty:
        raise ValueError(f"Producto {codigo} no existe en inventario.")
    return float(fila.iloc[0]["precio"])
...
def crear_venta(producto_codigo, cliente_codigo, cantidad, tabla):
    try:
        cantidad = float(cantidad)
        ...
        precio_unit = precio_producto_por_codigo(producto_codigo)

        if precio_unit < 0:
            raise ValueError("Precio inválido en inventario.")
        ...
        actualizar_stock(producto_codigo, cantidad)
...
def pestaña_ventas(notebook):
    ...
    cb_prod = ttk.Combobox(form, values=productos_vals, width=35, state="readonly")
    cb_cli  = ttk.Combobox(form, values=clientes_vals,  width=35, state="readonly")
    ent_cant = tb.Entry(form, width=10)
    ent_prec = tb.Entry(form, width=12, state="readonly")  # precio viene automático
    ...
    def poner_precio_segun_producto(_event=None):
        prod_txt = cb_prod.get().strip()
        if not prod_txt:
            return
        codigo_prod = _cod(prod_txt)
        try:
            precio = precio_producto_por_codigo(codigo_prod)
        except Exception as e:
            messagebox.showerror("Error", str(e))
            return

        ent_prec.config(state="normal")
        ent_prec.delete(0, tk.END)
        ent_prec.insert(0, str(precio))
        ent_prec.config(state="readonly")

    cb_prod.bind("<<ComboboxSelected>>", poner_precio_segun_producto)

    def llenar_campos_venta(_event=None):
        """Cuando selecciono una venta en la tabla, llenamos ID, cantidad, precio."""
        seleccion = tabla.focus()
        if not seleccion:
            return
        valores = tabla.item(seleccion, "values")
        if not valores:
            return
        vid, _fecha, prod_nom, cli_nom, cant, prec, _tot, _anulada = valores

        ent_id.delete(0, tk.END)
        ent_id.insert(0, str(vid))

        ent_cant.delete(0, tk.END)
        ent_cant.insert(0, str(cant))

        ent_prec.config(state="normal")
        ent_prec.delete(0, tk.END)
        ent_prec.insert(0, str(prec))
        # en edición de venta dejamos editable el precio
        # (no lo devolvemos a readonly aquí)

        cb_prod.set(prod_nom)
        cb_cli.set(cli_nom)
    ...
    tabla.bind("<<TreeviewSelect>>", llenar_campos_venta)

    pestaña.bind("<Visibility>", lambda _e: (listar_ventas(tabla), _recargar_combobox(), limpiar_campos_venta()))

    tb.Button(form, text="Crear venta", bootstyle="success",
              command=lambda: crear_venta(_cod(cb_prod.get()), _cod(cb_cli.get()), ent_cant.get(), tabla)
    ).grid(row=2, column=1, pady=6)

    tb.Button(form, text="Actualizar venta (opc.)", bootstyle="info",
              command=lambda: (lambda _id=_id_valido(): _id is not None and actualizar_venta(
                    tabla, _id,
    ...
    tb.Button(form, text="Refrescar", bootstyle="secondary",
              command=lambda: (listar_ventas(tabla), limpiar_campos_venta(), _recargar_combobox())
...
def enviar_mail_con_adjunto(destinatario: str, asunto: str, cuerpo: str, ruta_adjunto: Path):
    try:
        ...
        if not remitente or not password_app:
            raise ValueError("Faltan las credenciales GOOGLE_APP_EMAIL / GOOGLE_APP_PASS en las variables de entorno.")
        ...
        with open(ruta_adjunto, "rb") as f:
            parte = MIMEApplication(f.read(), Name=ruta_adjunto.name)
